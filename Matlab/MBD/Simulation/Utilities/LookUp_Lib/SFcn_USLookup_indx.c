...
#define S_FUNCTION_NAME SFcn_USLookup_indx
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    {
        case FXP_STORAGE_UINT16:
            (*((uint16_T *)pVoidOut1)) = usLookup_xy_us(((uint16_T *)pVoidIn1),*u1,Intervals);
            break;
        default:
