...
#define S_FUNCTION_NAME Sfun_usLookup_Rescale_xy_ss
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
/*****************************************************************************/
   ...
   DTypeId   y0DataType = ssGetInputPortDataType(S, 0);
   uint16_T          *pY0  = (uint16_T *)ssGetOutputPortSignal(S,0);
    ...
    switch(y0DataType)
    {
        case SS_INT16:
        {
			...
			InputInt16PtrsType  pU0  = (InputInt16PtrsType)u0ptrs;
			InputInt16PtrsType  pU1  = (InputInt16PtrsType)u1ptrs;
