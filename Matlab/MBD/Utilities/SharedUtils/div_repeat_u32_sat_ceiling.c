/*
 * File: C:\cvs\BMW_DPH_Layer\BMW_Projects\B38_PST1\Output\Autocode\slprj\ert\_sharedutils\div_repeat_u32_sat_ceiling.c
 *
...
uint32_T div_repeat_u32_sat_ceiling(uint32_T numerator, uint32_T denominator,
 uint32_T nRepeatSub)
{
  uint32_T quotient;
  uint32_T iRepeatSub;
  ...
  if(denominator == 0) {
    ...
    if(overflow || (tempQuotient >= 0xFFFFFFFFU)) {
